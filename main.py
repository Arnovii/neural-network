...
Requisitos:
    pip install numpy matplotlib torchvision mplcursors
"""
...
def run_interactive_mode() -> None:
    ...
    class FederatedLearningApp:
        ...
        def _create_ui(self) -> None:
            ...
            ttk.Label(ctrl, text="Parámetros", font=("Helvetica", 14, "bold")).pack(pady=10)
            ...
            def _add_slider(parent, label, var, lo, hi):
                ...
                ttk.Scale(parent, from_=lo, to=hi, orient=tk.HORIZONTAL,
                          variable=var, length=200, command=_snap_int(var)).pack(fill=tk.X, pady=5)
            ...
            ttk.Label(ctrl, text="Ejemplos de entrenamiento:").pack(anchor=tk.W, pady=(10, 0))
            ...
            btn_run = ttk.Button(ctrl, text="Ejecutar Experimento", command=self._run_experiment)
            btn_run.pack(fill=tk.X, pady=5)
            ToolTip(btn_run, "Ejecuta el algoritmo con los parámetros actuales")

            btn_compare = ttk.Button(ctrl, text="Comparar Configuraciones", command=self._compare_configurations)
            btn_compare.pack(fill=tk.X, pady=5)
            ToolTip(btn_compare, "Ejecuta un nuevo experimento y lo superpone\ncon los resultados actuales.")

            btn_clear = ttk.Button(ctrl, text="Limpiar Gráficos", command=self._clear_plots)
            btn_clear.pack(fill=tk.X, pady=5)
            ToolTip(btn_clear, "Borra todas las gráficas actuales")

            btn_save = ttk.Button(ctrl, text="Guardar Resultados", command=self._save_results)
            ...
            self.fig.suptitle("Análisis de Algoritmo de Diego", fontsize=14, fontweight="bold")
        ...
        def _create_progress_window(self, num_experiments, num_epochs):
            ...
            ttk.Label(win, text="Entrenamiento en progreso",
            ...
            exp_bar = ttk.Progressbar(win, orient=tk.HORIZONTAL, length=470,
            ...
            epoch_bar = ttk.Progressbar(win, orient=tk.HORIZONTAL, length=470,
            ...
            ttk.Label(win, textvariable=msg_var, font=("Helvetica", 9),
                      foreground="#555555", wraplength=470, justify=tk.LEFT).pack(anchor=tk.W, padx=24)

            return {"window": win, "exp_label_var": exp_label_var,
                    "exp_bar": exp_bar, "epoch_bar": epoch_bar, "msg_var": msg_var}

        def _poll_queue(self, q, widgets, num_experiments, num_epochs, on_done):
            import queue as queue_module
            try:
                while True:
                    msg_type, payload = q.get_nowait()
                    if msg_type == "exp":
                        widgets["exp_label_var"].set(f"Experimento {payload} de {num_experiments}")
            ...
            self.root.after(100, lambda: self._poll_queue(q, widgets, num_experiments, num_epochs, on_done))
        ...
        def _run_experiment(self):
            ...
            self.root.after(100, lambda: self._poll_queue(q, widgets, num_experiments, num_epochs, _on_done))

        def _compare_configurations(self):
            if not self.current_results:
                messagebox.showwarning("Advertencia", "Primero ejecuta un experimento")
                return
            self.previous_results.append({"results": self.current_results, "params": self.current_params})
            self.color_index += 1
        ...
        def _save_results(self):
            ...
            with open(filename, "w") as f:
                json.dump({
                    ...
                    "results": {k: self.current_results[k] for k in
                                ("timestamp", "final_mean_accuracy", "final_std_accuracy", "all_histories")},
        ...
        def _plot_results(self, results, params, comparison=False):
            ...
            line1, = ax1.plot(acc["x"], acc["y_mean"], "o-", color=color,
                              linewidth=2, markersize=5, label=label)
            ax1.fill_between(acc["x"], acc["y_lower"], acc["y_upper"], alpha=0.2, color=color)
            ax1.set(xlabel=acc["xlabel"], ylabel=acc["ylabel"], title="Evolución del Promedio (±1σ)")
            ...
            def _fmt_acc(sel):
                ...
                sel.annotation.get_bbox_patch().set(facecolor="#ffffcc", alpha=0.95, edgecolor="#888")
            ...
            if part:
                for p in part["partitions"]:
                    ln, = ax2.plot(p["x"], p["y"], "o-", label=f"Partición {p['id']}",
                                   alpha=0.7, markersize=4)
                    part_lines.append(ln)
            ax2.set(xlabel=part.get("xlabel", "Época"), ylabel=part.get("ylabel", "Precisión (%)"),
            ...
            def _fmt_part(sel):
                ...
                sel.annotation.get_bbox_patch().set(facecolor="#e8f5e9", alpha=0.95, edgecolor="#888")
            self._add_cursor(part_lines, _fmt_part)

            # Panel 3 — Distribución de precisión final
            dist = prepare_distribution_data(histories)
            bin_centers = [(dist["bins"][i] + dist["bins"][i + 1]) / 2
                           for i in range(len(dist["bins"]) - 1)]
            width = (dist["bins"][1] - dist["bins"][0]) * 0.9 if len(dist["bins"]) > 1 else 1.0
            bars = ax3.bar(bin_centers, dist["counts"], width=width,
                           color=color, alpha=0.7, edgecolor="black")
            ax3.axvline(dist["mean"], color="red", linestyle="--", linewidth=2, label="Promedio")
            ...
            def _fmt_dist(sel):
                ...
                sel.annotation.get_bbox_patch().set(facecolor="#fff3e0", alpha=0.95, edgecolor="#888")
            ...
            def _fmt_conv(sel):
                ...
                sel.annotation.get_bbox_patch().set(facecolor="#e3f2fd", alpha=0.95, edgecolor="#888")
        ...
        def _plot_comparison(self):
            ...
            labels = [f"P={p['params']['num_partitions']}, E={p['params']['num_epochs']}"
