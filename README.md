...
### 1. CNN Extractor: `torch.inference_mode()` (cnn_extractor.py:673)

**Cambio:** Reemplazó `torch.no_grad()` por `torch.inference_mode()` en la función `extract()`.
...
**Beneficio:** `inference_mode()` deshabilita el seguimiento de versiones de tensores además de los gradientes, resultando en ~5-10% de mejora en velocidad de inferencia sin riesgo (la CNN ya tiene `requires_grad=False` y está en `.eval()`).

---

### 2. MLP: Optimización de Memoria en `forward_and_gradients()` (mlp.py:294-295)

**Cambio:** Eliminó la matriz one-hot innecesaria reutilizando `A3` con operaciones in-place.

```python
# Antes
Y_onehot = np.zeros_like(A3)           # alloc 1: (10, N)
Y_onehot[Y, np.arange(N)] = 1.0
delta3 = A3 - Y_onehot                 # alloc 2: (10, N)

# Después
delta3 = A3.copy()                     # 1 alloc: (10, N)
delta3[Y, np.arange(N)] -= 1.0
```

**Beneficio:** Reduce de 2 allocations de memoria a 1 por época. Con batches de 5000+ imágenes, la matriz es de ~200 KB; multiplicado por 10 épocas × N Workers, ahorra MBs de asignaciones innecesarias.

---

### 3. MLP: Optimizaciones en `apply_gradients()` (mlp.py:416-434)

**Cambio 1 – Lógica explícita de momentum:**

```python
# Antes: silencia errores (momentum > 0 con velocities=None cae al camino SGD puro)
if momentum == 0.0 or velocities is None:
    # SGD puro
    return

# Después: genera error explícito
if momentum == 0.0:
    # SGD puro
    return
...
**Beneficio:** Evita bugs silenciosos donde el PS intenta usar momentum sin mantener el estado.

**Cambio 2 – Operaciones in-place en momentum:**

```python
# Antes: 2 arrays temporales por parámetro
velocities[k] = momentum * velocities[k] + gradients[dk]

# Después: completamente in-place
velocities[k] *= momentum
velocities[k] += gradients[dk]
```

**Beneficio:** Con 6 parámetros y 100 épocas, elimina 1200 allocations temporales innecesarias.

**Cambio 3 – Inicialización lazy fuera del loop:**

```python
# Ahora: se ejecuta una sola vez al inicio de momentum
for k in keys:
    if k not in velocities:
        velocities[k] = np.zeros_like(params[k])
```

**Beneficio:** Reduce overhead de comprobación `k not in velocities` en cada época (aunque mínimo, mejora la claridad).

---

### 4. Distributed: Aumento de Muestra de Entrenamiento (5000 → 10000)

**Cambios:**
- `parameter_server.py:345`: `n_samples: int = 10000`
