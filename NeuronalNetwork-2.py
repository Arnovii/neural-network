        ...
        if not os.path.exists(archivo):
            ...
            try:
                ...
                try:
                    ...
                    raise Exception("No se pudieron descargar los datos. Verifica tu conexión.")
...
def cargar_imagenes(archivo_gz):
    ...
    with gzip.open(archivo_gz, 'rb') as f:
        ...
        if magic != 2051:
            raise ValueError(f'Magic number incorrecto: esperado 2051, obtenido {magic}')
...
def cargar_etiquetas(archivo_gz):
    ...
    with gzip.open(archivo_gz, 'rb') as f:
        ...
        if magic != 2049:
            raise ValueError(f'Magic number incorrecto: esperado 2049, obtenido {magic}')
...
def main():
    ...
    for i in range(5):
        ...
        real = Y_test[i]
        print(f"  Imagen {i}: Predicción={pred}, Real={real}, {'✓' if pred==real else '✗'}")
