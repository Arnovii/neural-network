...
def partition_mnist_data_simple(
    ...
    # Crea sub-listas vacías que almacenarán los índices para cada partición
    partitions_indices: List[List[int]] = [[] for _ in range(num_partitions)]

    for digit in range(10):
        # Devuelve los índices donde la etiqueta es igual a "digit"
        # np.where() retorna una tupla de arrays
        indices = np.where(Y_train == digit)[0]

        # np.random.shuffle mezcla los índices aleatoriamente in-place
        np.random.shuffle(indices)

        # Realiza una distribución Round Robin
        for i, idx in enumerate(indices):
            partitions_indices[i % num_partitions].append(idx)

    result = []

    # Convierte índices en datos reales
    for i, part_idx in enumerate(partitions_indices):
        part_idx_arr = np.array(part_idx)

        # Evita que la partición quede ordenada por clases
